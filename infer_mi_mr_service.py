...
for i, row in df.iterrows():
    ...
    if labs:
        ...
        if len(labs) >= K:
            ...
            need_model.append((asset_name, model_name, purpose, len(labs)))  # 记录需要补全的数量
    else:
        # 规则未命中，需要模型预测
        if use_fallback:
            need_model.append((asset_name, model_name, purpose, 0))  # 0表示完全由模型预测
            candidates.append([])
        else:
            candidates.append([''] * K)
            need_model.append(None)
...
if use_fallback and any(x is not None for x in need_model):
    ...
    if model_inputs:
        ...
        # 批量预测
        model_predictions = model_predict(asset_names_model, models_model, purposes_model)
        
        # 将模型预测结果填入candidates
        model_idx = 0
        for i, (candidate, model_input) in enumerate(zip(candidates, need_model)):
            if model_input is not None:
                existing_count = model_input[3]  # 已有的规则结果数量
...
for i in range(min(5, len(df))):
    if i < len(need_model) and need_model[i] is not None:
        ...
        method = "规则"
    logger.info(f"  样本{i+1}: {df['资产名称'].iloc[i]} | {df['型号'].iloc[i]} | {df['用途'].iloc[i]} -> {df['top1'].iloc[i]} ({method})")